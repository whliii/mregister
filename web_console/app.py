...
UI_TRANSLATIONS = {
    "zh-CN": {
        ...
        "section_docs": "接口文档",
        "docs_intro_title": "说明",
        "docs_intro_desc": "外部 API 默认使用站点里配置的默认 GPTMail、默认 YesCaptcha 和默认代理。API 创建的任务支持自动删除，当前策略为任务完成后 24 小时自动清理。",
        ...
        "docs_response_desc": "`completed_count` 表示任务当前真实完成数。只有任务完成并且压缩包生成后，查询接口才会返回 `download_url`。API 创建的任务会在 `auto_delete_at` 指定时间后自动删除。",
        "table_method": "方法",
        "table_path": "路径",
        "table_desc": "说明",
        "table_field": "字段",
        "table_type": "类型",
        "table_required": "必填",
        "endpoint_create_desc": "创建一个新的 API 任务",
        "endpoint_query_desc": "查询任务状态、已完成数量和下载地址",
    ...
    "en": {
        ...
        "docs_intro_desc": "External API calls use the site's default GPTMail, default YesCaptcha, and default proxy configuration. API-created tasks are automatically removed 24 hours after completion.",
        "docs_endpoints_title": "Endpoints",
        "docs_create_params_title": "Create task parameters",
        "docs_create_example_title": "Create task example",
        "docs_query_example_title": "Query task example",
        "docs_download_example_title": "Download example",
        "docs_response_title": "Response notes",
        "docs_response_desc": "`completed_count` is the real completed quantity. `download_url` is only returned after the task finishes and its archive is ready. API-created tasks are removed at `auto_delete_at`.",
        "table_method": "Method",
        "table_path": "Path",
        ...
        "endpoint_create_desc": "Create a new API task",
        "endpoint_query_desc": "Query task status, completed quantity, and download URL",
        "endpoint_download_desc": "Download the task result archive",
        "required_yes": "Yes",
        "required_no": "No",
        ...
        "param_use_proxy_desc": "Whether to use the default proxy. Omit or set false to disable proxy use",
        "param_concurrency_desc": "Concurrency value. Defaults to 1",
