import os, random, string, time, re, struct
import threading
...
proxy_url = os.getenv("GROK_PROXY_URL", "").strip()
...
output_dir = Path(os.getenv("GROK_OUTPUT_DIR") or os.getcwd()).resolve()


def should_stop() -> bool:
    return stop_event.is_set() or (target_count > 0 and success_count >= target_count)


def keys_dir() -> Path:
    return output_dir / "keys"
...
def register_single_thread(worker_index: int):
    # 错峰启动，防止瞬时并发过高
    ...
    while not should_stop():
        try:
            with requests.Session(impersonate="chrome120", proxies=PROXIES) as session:
                ...
                mailbox = email_service.acquire_mailbox(worker_index)
                if not mailbox:
                    print(f"[-] 线程-{threading.get_ident()} 建箱失败，等待 5s...")
                ...
                if not send_email_code_grpc(session, email):
                    ...
                    email_service.report_failure(mailbox, "send_email_code_failed", worker_index=worker_index)
                    time.sleep(5); continue
                
                # Step 2: 获取验证码
                verify_code = email_service.wait_for_verification_code(mailbox, timeout=120)
                if not verify_code:
                    print(f"[-] {email} 未收到验证码")
                    email_service.report_failure(mailbox, "otp_timeout", worker_index=worker_index)
                    continue
                email_service.report_otp_received(mailbox, worker_index=worker_index)

                # Step 3: 验证验证码
                if not verify_email_code_grpc(session, email, verify_code):
                    print(f"[-] {email} 验证码无效")
                    email_service.report_failure(mailbox, "otp_invalid", worker_index=worker_index)
                    continue
                ...
                for attempt in range(3):
                    ...
                    if res.status_code == 200:
                        ...
                        if match:
                            ...
                            if sso:
                                with file_lock:
                                    task_keys_dir = keys_dir()
                                    task_keys_dir.mkdir(parents=True, exist_ok=True)
                                    with open(task_keys_dir / "grok.txt", "a", encoding="utf-8") as f:
                                        f.write(sso + "\n")
                                    with open(task_keys_dir / "accounts.txt", "a", encoding="utf-8") as f:
                                        f.write(f"{email}:{password}:{sso}\n")
                                    global success_count
                                    success_count += 1
                                    avg = (time.time() - start_time) / success_count
                                    print(f"[✓] 注册成功: {email} | SSO: {sso[:15]}... | 平均: {avg:.1f}s")
                                    email_service.report_account_success(mailbox, worker_index=worker_index)
                                break  # 跳出 for 循环，继续 while True 注册下一个
                    
                    print(f"[-] {email} 提交失败 ({res.status_code})")
                    if attempt == 2:
                        email_service.report_failure(mailbox, f"submit_failed:{res.status_code}", worker_index=worker_index)
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=t) as executor:
        ...
        futures = [executor.submit(register_single_thread, index + 1) for index in range(t)]
        try:
